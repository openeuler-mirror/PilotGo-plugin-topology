<template>
  <div id="topo-container" class="container">
    <div class="topo_legend">
      <p>
        <img width="20" src="../assets/icon/server_blue.png" alt="" /><span>主机</span>
      </p>
...
onMounted(() => {
  ...
  document
    .getElementById("topo-container")!
    .addEventListener("click", function (_event: any) {
      // 点击画布时，关闭日志弹窗和tab页面
...
const updateTopoData = (topoData: any) => {
  ...
  if (!combo_color || combo_color.size < topo_data.combos.length) {
    ...
    topo_data.combos.forEach((combo: any, i: number) => {
      combo_color.set(combo.id, [
        colorSets[i].mainStroke as string,
        colorSets[i].mainFill as string,
      ]);
    });
  }
...
const menu = new G6.Menu({
  ...
  handleMenuClick(_target, item: any) {
    if (item._cfg) {
      let host_name = "" as any;
      let process_name = "" as any;
      let target_ip = "" as any;
      ...
      if (node_type === "process") {
        ...
        target_ip = nodeNeighbors[0]._cfg.model.metrics.label;
      } else if (node_type === "host") {
        process_name = host_name = item._cfg.model!.name;
        target_ip = item._cfg.model!.label;
      }
      ...
      useTopoStore().node_click_info.target_ip = target_ip;
    }
  },
});
...
function initGraph(data: any) {
  ...
  graph.on("node:dragend", (e) => {
    node_position_process.set(e.item?.getModel().id as string, [
      e.item?.getModel().x as number,
      e.item?.getModel().y as number,
    ]);
    if (e.item?.getModel().id === "3df830f0-cc71-48b4-8693-cd36098cc6d1_process_147139") {
      console.log(
        "node drag end: ",
        node_position_process.get(e.item?.getModel().id as string)
      );
    }
  });

  graph.on("combo:dragend", (e) => {
    combo_positions.set(e.item?.getModel().id as string, [
      e.item?.getModel().x as number,
      e.item?.getModel().y as number,
    ]);
  });
  ...
  graph.on("combo:dblclick", (e) => {
    combo_collapse_status.set(
      e.item?.getID() as string,
      e.item?.getModel().collapsed as boolean
    );
  });
...
function changeZoom(zoom: number) {
  const point_position = graph.getPointByCanvas(
    graph.getWidth() / 2,
    graph.getHeight() / 2
  );
